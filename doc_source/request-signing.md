...
The following example uses the Elasticsearch low\-level Java REST client to perform two unrelated actions: registering a snapshot repository and indexing a document\. If you want to try the new OpenSearch client instead of the legacy Elasticsearch one, see [opensearch\-java](https://github.com/opensearch-project/opensearch-java) on GitHub\. You must provide values for `region` and `host`\.
